...
import com.lkroll.ep.mapviewer.data.{ Aerostat => AerostatData, SyncOrbitStation => SyncOrbitStationData, AstronomicalObject }
...
class SyncOrbitStation(val settlement: SyncOrbitStationData) extends GraphicsObject with Overlayed {
...
object SyncOrbitStation {
  ...
  def fromData(data: SyncOrbitStationData): SyncOrbitStation = {
    new SyncOrbitStation(data)
  }
}
