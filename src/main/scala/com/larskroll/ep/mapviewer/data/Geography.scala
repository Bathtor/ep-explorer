...
abstract class AtmosphericObject(val pos: Geography.Position, val height: Length, val location: Rotating, name: String, id: UUID) extends AstronomicalObject(name, id) {
    val position = EquatorialPosition(location, pos.rightascension, pos.declination, location.radius + height);
}

abstract class SurfaceObject(pos: Geography.Position, location: Rotating, name: String, id: UUID) extends AtmosphericObject(pos, Meters(0.0), location, name: String, id: UUID) {

}

class Settlement(name: String, id: UUID, pos: Geography.Position, location: Rotating, val size: Length,
                 val allegiance: Allegiance, val langs: Seq[Language], val industries: Seq[Industry])
        extends SurfaceObject(pos, location, name: String, id: UUID) {
    override def `type` = "Settlement";
}

class PandoraGate(name: String, id: UUID, pos: Geography.Position, location: Rotating,
                 val allegiance: Allegiance)
        extends SurfaceObject(pos, location, name: String, id: UUID) {
    override def `type` = "Pandora Gate";
}

class Aerostat(name: String, id: UUID, pos: Geography.Position, height: Length, location: Rotating,
               val allegiance: Allegiance, val langs: Seq[Language], val industries: Seq[Industry])
        extends AtmosphericObject(pos, height, location, name: String, id: UUID) {
    override def `type` = "Aerostat";
}

class SyncOrbitStation(name: String, id: UUID, pos: Geography.Position, height: Length, location: Rotating,
               val allegiance: Allegiance, val langs: Seq[Language], val industries: Seq[Industry])
        extends AtmosphericObject(pos, height, location, name: String, id: UUID) {
