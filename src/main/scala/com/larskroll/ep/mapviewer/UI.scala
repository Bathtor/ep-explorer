...
object TimeControls extends Logging {
    ...
    val block = div(id := "time",
        table(tr(th("Time"), th("Day"), th("Year"), th("Epoch"), th("")),
            tr(td(hourField, ":", minuteField, ":", secondField), td(dayField), td(yearField), td(epoch), td("TT"))),
        span("Speed", cls := "color-primary-0", style := "font-weight: bold;"), speed, play, stop, step).render;
}
...
object UI extends Logging {
    ...
    track.onclick = (e: MouseEvent) => {
        if (!search.value.isEmpty()) {
            searchObject(search.value) match {
                case Some(obj) => {
                    Main.scene match {
                        case Some(scene: Tracking) => scene.track(obj); search.value = ""
    ...
    infoButton.onclick = (e: MouseEvent) => {
        if (!search.value.isEmpty()) {
            searchObject(search.value) match {
                case Some(obj) => {
                    Main.scene match {
                        case Some(scene: Selecting) => scene.select(obj); search.value = ""
    ...
    val top = div(id := "topmenu",
        ...
        span(b("Currently Tracking: "), id := "tracking"), trackingContent,
        div(id := "search", span("Search", cls := "color-primary-0", style := "font-weight: bold;"), data, search, track, infoButton));
